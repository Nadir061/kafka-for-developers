...
6) Конфигурация:
  ...
  - batch.size максимальный размер батча (по умолчанию: 16384 (16 КБ)), props.put("batch.size", 32768); // 32 КБ
  - linger.ms - время ожидания батчем новых сообщений перед отправкой: props.put("linger.ms", 0); // отправлять сразу, props.put("linger.ms", 10); // 10 миллисекунд
